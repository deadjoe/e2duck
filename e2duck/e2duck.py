    ...
    def connect_db(self):
        ...
        try:
            ...
            try:
                ...
                logging.info(f"Based on system memory ({total_memory:.1f}GB) setting DuckDB memory limit to {memory_limit}")
                ...
                try:
                    ...
                    if total_rows is None:
                        try:
                            ...
                            logging.error(f"Failed to get worksheet '{sheet_name}' row count: {str(e)}")
                            return None
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    try:
                        ...
                        logging.error(f"Error processing worksheet '{sheet_name}' result: {str(e)}")
    ...
    def import_data(self, batch_size=DEFAULT_BATCH_SIZE):
        ...
        try:
            ...
            if total_memory_gb >= 64:
                max_batch_size = 200000  # 64GB and above memory
            elif total_memory_gb >= 32:
                max_batch_size = 100000  # 32GB memory
            elif total_memory_gb >= 16:
                max_batch_size = 50000   # 16GB memory
            elif total_memory_gb >= 8:
                max_batch_size = 20000   # 8GB memory
            else:
            ...
            if batch_size < max_batch_size:
                ...
                logging.info(f"Based on system memory ({total_memory_gb:.1f}GB) adjusting batch size to {max_batch_size} rows")
        ...
        for sheet_name, info in self.sheets_info.items():
            ...
            try:
                logging.info(f"Starting import of worksheet '{sheet_name}' to temporary table '{temp_table_name}'")
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                logging.info("Using fallback: row-by-row insertion for data")
                                for _, row in batch_df.iterrows():
                                    values = []
                                    placeholders = []
                                    column_list = []
                        ...
                        logging.info(f"Batch {i+1}/{num_batches}: Imported {rows_imported}/{total_rows} rows to table '{temp_table_name}'")
                ...
                error_message = str(e)
                logging.error(f"Error importing data to table '{temp_table_name}': {error_message}")
    ...
    def validate_import(self):
        ...
        for sheet_name, info in self.sheets_info.items():
            ...
            try:
                            ...
                            for col_name, stats in info['column_stats'].items():
                                ...
                                if 'min' in stats and 'max' in stats:
                                    ...
                                    try:
                                        ...
                                        validation['messages'].append(
                                            f"Error validating column '{col_name}' statistics: {str(e)}"
                                        )
                                    ...
                                    try:
                                        ...
                                        validation['messages'].append(
                                            f"Error validating column unique value: {str(e)}"
                                        )
                ...
                if db_count == 0:
                   ...
                   all(dtm['match'] for dtm in validation['data_types_match']):
                    ...
                    if self.safe_mode or all(sv['status'] == 'success' for sv in validation['stats_verification'] if 'status' in sv):
                        ...
                        validation['messages'].append("Data validation mostly successful, but some statistics mismatch detected")
                else:
                    validation['overall_status'] = 'error'
                    if not validation['messages']:
                ...
                logging.error(f"Error validating table '{table_name}': {str(e)}")
                    ...
                    if 'unique_count' in col_stats and \
                       ...
                       col_stats['unique_count'] < info['row_count'] / 10:
                        ...
                        try:
                            ...
                            logging.info(f"Created index for table '{table_name}' column '{col_name}'")
                        except Exception as e:
                            logging.warning(f"Cannot create index for table '{table_name}' column '{col_name}': {str(e)}")

                logging.info(f"Table '{table_name}' optimization completed")

            except Exception as e:
                logging.error(f"Error optimizing table '{table_name}': {str(e)}")
